...
def _run_mean_nprr(args):
    store_path, n_range, dataset, eps, k, num_runs, seed = args

    method = GroupMeanWaudbySmith
    ...
    for i in range(num_runs):
        for n in n_range:
            ...
            try:
                ...
                mechanism = method(eps=eps, rng=rng, input_range=input_range, k=k)
...
def simulate_group_mean_nprr(output_path, use_mpi=False, i=None, fixed_group_size=False) -> None:
    """
    ...
    # Create WaudbySmith classes for different k
    k_range = [1, 2, 3, 4, 8, 16, 32][::-1]
    ...
    for k in k_range:
        for dataset in datasets:
            for num_groups in num_groups_range:
                ...
                store_path = output_path / str(k) / dataset.__name__ / str(num_groups)
    ...
    # Run the simulation
    simulation_loop(_run_mean_nprr, tasks, use_mpi=use_mpi)
