...
class KGroupsExtremeEqualSpread(SyntheticContinuousGroupData):
    ...
    def __init__(  # noqa: PLR0913
        ...
        for i in range(num_groups):
            ...
            # Generate data only at the extremes of the input range, but ensure the correct mean
            p = (means[i] - input_range[0]) / (input_range[1] - input_range[0])
            data_ = data_rng.binomial(1, p, samples_per_group)
            data_ = data_ * (input_range[1] - input_range[0]) + input_range[0]

            data[start : start + samples_per_group, 0] = i
            data[start : start + samples_per_group, 1] = data_
